...
namespace ImGuiOverlay.Input
{
    ...
    public class Device
    {
        ...
        // optional: let other code subscribe to button edgesaa
        ...
        private const string MAKCU_FRIENDLY_NAME = "USB-SERIAL CH340";
        ...
        public static bool AutoConnectMakcu()
        {
            try
            {
                Console.WriteLine("[Device] Scanning for CH343/CH340...");
                using var searcher = new System.Management.ManagementObjectSearcher(
                    "SELECT * FROM Win32_PnPEntity WHERE Name LIKE '%CH343%' OR Name LIKE '%CH340%'");
                ...
                foreach (var obj in searcher.Get())
                {
                    ...
                    if (m.Success)
                    {
                        Console.WriteLine($"[Device] Found on COM{m.Groups[1].Value}: {name}");
                        Connect($"COM{m.Groups[1].Value}");
                        return true;
                    }
                }
                Console.WriteLine("[Device] No CH343/CH340 found.");
                return false;
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[Device] AutoConnect error: {ex.Message}");
                return false;
            }
        }
        /// <summary>
        ...
        /// </summary>
        public static bool AutoConnectMakcu1()
        {
            try
            {
                string com =
